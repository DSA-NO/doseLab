...
SRC_DIR="${DOSELAB_CALIB_SOURCE_DIR:-analysis/calibration/latest}"
BASE_DIR="${DOSELAB_CALIB_BASELINE_DIR:-analysis/calibration/baselines}"
STAMP="${1:-$(date -u +%Y%m%d-%H%M%S)}"
TARGET_DIR="${BASE_DIR}/${STAMP}"
INCLUDE_ROOT="${DOSELAB_CALIB_INCLUDE_ROOT:-0}"
...
if [[ "${INCLUDE_ROOT}" == "1" ]]; then
  shopt -s nullglob
  root_files=("${SRC_DIR}"/doseLab-run-calib-v1-*.root)
...
echo "Archived calibration baseline to ${TARGET_DIR}"
